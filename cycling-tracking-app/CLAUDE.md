...
- **滞留(stalled)判定はアプリ側で実施**: 直近5分の移動距離が250m未満なら滞留とみなす(`STALLED_WINDOW_MS`/`STALLED_DISTANCE_THRESHOLD_M`)。**滞留に入った瞬間・滞留から復帰した瞬間のみ送信し、滞留継続中は送信しない**(電力節約、ユーザー指示)。アプリ起動時(再起動時の自動復帰を含む)には`clearTrail()`で軌跡・ライド開始時刻をリセットする(2026-08-10修正、旧セッションの古いデータ持ち越しによる誤判定を防止)。「ロスト」(通信断)はサーバー側の別概念で、直近が稼働中と分かっていたのに10分間無音の場合のみ判定する(滞留由来の意図的な無音とは区別、2026-08-10修正。詳細は上記「現在の状態」参照)。
