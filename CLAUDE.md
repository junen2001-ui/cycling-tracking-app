...
## 次に取り組むこと
- **実機未検証(次回最優先)**: 2026-08-10夜に、(1)アプリ再起動時に滞留状態・軌跡がクリアされない不具合の修正(`App.js`起動時に`clearTrail()`を呼ぶよう変更)、(2)現在地矢印マーカーのデザイン改善(白いパック+鋭角の矢印、`RouteMapScreen.js`)を実施し`mobile-preview-sharp-arrow-20260810.apk`をビルドしたが、**まだ実機で再検証していない**。詳細は `progress/progress.md` の2026-08-10セクション「続き(同日夜)」参照。
...
- 現時点での最新ビルド: `mobile-preview-sharp-arrow-20260810.apk`(ローカルビルド、`cycling-tracking-app/build/`配下・Git管理対象外、**実機未検証**)。位置情報送信・滞留/ロースト判定の全面見直しに加え、アプリ再起動時の軌跡/滞留状態クリア漏れ修正・矢印デザイン改善を含む。前バージョン(`mobile-preview-stalled-lost-20260809.apk`)は2026-08-10に終日の実機検証で、移動中の継続送信・滞留検知・滞留中の送信抑制・稼働再開の検知・アプリ終了時の送信停止まで確認済み(併せてサーバー側の未反映バグとロースト判定タイミングも修正・本番デプロイ済み)。詳細は `progress/progress.md` の2026-08-10セクション参照。
...
- **データ不足による誤った滞留判定の防止(2026-08-25修正)**: 以前は「ライド開始時刻から5分経過したか」でガードしていたが、この時刻は新規ログイン時にしかリセットされず、通信断からの復帰や「自動送信のOFF→ON」など新規ログインを経ずに追跡が再開されるケースで、実データがほぼ無いのに即座に滞留と誤判定するバグが実イベントで発生した(`src/mobile/src/route/trailStorage.js`)。「直近5分の時間窓内で一番古いデータが実際にどれだけ過去まで遡れているか」で判定するよう修正し、原因を問わず頑健にした。**この修正はまだローカルビルド・実機検証未了**(詳細は `progress/progress.md` の2026-08-25セクション参照)。
