...
## 現在の状態
- **コース制(2026-09-01実装、詳細は`progress/progress.md`参照)**: 短距離・中距離・長距離の3コースに対応済み。`courses`テーブル(slug/name/ゼッケン接頭辞/出走時刻/ゴール地点)、参加者は`course_id`・`bib_number`・`goal_time`を持つ。管理画面は`admin.html?course=short`のようにコース単位で開く(コース指定が無いとエラー表示)。参加者の同一性は電話番号ではなく「コース+ゼッケン番号」で判定する(家族間の電話番号共有への対応、実データ検証で判明)。**未登録の電話番号ではログインできない**(`/api/auth/verify-code`が401を返す)ため、Excelインポートまたは管理者の手動紐付け(`POST /api/participants/:id/phone`)が事前に必要。ゴール判定(コース公式スタート時刻+`GOAL_MIN_ELAPSED_MS`〈実機テストのため15分に短縮中、**本番前に1時間へ戻すこと**〉後、ゴール地点200m以内で記録)・コース逸脱アラート(ルートから50m超・2回連続で通知、実機テストのため3分継続方式から変更。休憩所内は抑制)も実装済み。「現在逸脱中かどうか」は`participants.deviation_alerted_at`として永続化しており(2026-09-03)、参加者一覧に常時バッジ表示・通知パネルも画面読み込み時に現在の状態をバックフィルする(見逃し対策)。コース逸脱時は`expo-notifications`による通知欄への通知も追加済み(フルスクリーンインテントは実機ソース確認の上、現状のライブラリでは非対応と判断し見送り)。管理画面にテスト用の「全参加者データをリセット」ボタンあり(ゴール済みがいてもインポートロックを解除できる)。**本番サーバーにもデプロイ済み**(2026-09-02〜03)。Excelインポートのコース列は完全一致に加え「ショート/ミドル/ロング」等のキーワード部分一致でも判定する(`resolveCourseSlug()`)。インポートは既存データを全消去してから登録する総入れ替え方式(ゴール済みがいるとロック)。**管理画面のPlaywright検証・運営マニュアル作成は未実施。モバイルは`mobile-preview-notifications-20260903.apk`(送信間隔30秒・逸脱2回連続判定・expo-notifications反映)をユーザーが実機で確認し、Excelインポートが正常動作することを確認済み**。
...
## 次に取り組むこと
- **最優先(2026-09-03時点)**: (1)`GOAL_MIN_ELAPSED_MS`(現在15分)とコース逸脱の「2回連続」判定・送信間隔30秒は、いずれも実機テストのための暫定値。**実イベント前に本来の値(ゴール判定1時間、送信間隔60秒、逸脱は要件次第で時間ベースに戻す等)へ見直すこと**。(2)管理画面のPlaywright検証(コース別URL絞り込み・拡張Excelインポートダイアログ・Excelエクスポート・ゴール済み折りたたみの見た目/操作確認)、(3)運営マニュアル(Artifact)の作成(3コース分の画面URL・出走時刻設定・**受付での電話番号手動紐付け手順**・「全参加者データをリセット」ボタンの扱い〈テスト専用、実イベント中は絶対に押さないよう明記〉・ゴール済み一覧/コース逸脱アラートの見方・Excelエクスポート手順を含む)。いずれもユーザーの明示的な指示があれば着手する。詳細は`progress/progress.md`の2026-08-28〜09-03セクションと計画ファイル`C:\Users\jun_e\.claude\plans\idempotent-riding-cascade.md`を参照。
...
- 実機確認済みの最新ビルド: `mobile-preview-notifications-20260903.apk`(送信間隔30秒・コース逸脱2回連続判定・`expo-notifications`による通知欄通知〈`AndroidImportance.MAX`チャンネル+振動パターン〉を反映。ユーザーが実機で確認し、Excelインポートが正常動作することも確認済み)。詳細は `progress/progress.md` の2026-08-25〜2026-09-03の各セクション参照。
