...
## 2026-08-20: keywordの関連度マッチに乗らない店(早朝営業の一般飲食店等)を拾えるようにtype=restaurant検索を統合
ユーザー報告: 久留米市江戸屋敷起点・土曜7:00・希望距離20kmで「三福ラーメン」(八女市蒲原)が候補に入らない。

### 原因
`searchNearbyCafes()`は`keyword=モーニング breakfast 朝食 朝ごはん 朝御飯 朝ご飯`によるテキスト関連度マッチのみで候補を絞り込んでいた。三福ラーメンは土曜8:00〜15:00営業で到着予想時刻(8:20頃)には開いており、直線距離も検索半径内(9.1km)だが、店名・レビュー等がGoogle側の「朝食/モーニング」関連度判定に乗っていないため、keyword検索の結果(51件)に一件も含まれていなかった。同じ地点・半径で`keyword`を外し`type=restaurant`のみで検索すると60件中に含まれることを確認した。つまり、早朝から営業していても「朝食専門店」として認識されていない一般の飲食店(ラーメン店・食堂等)は、keywordによる事前フィルタの時点で候補プールに入れない設計になっていた。

### 検討・意思決定
対応案として(A)keyword検索に加えてtype=restaurant(keyword無し)でも別途検索しマージ、(B)keywordに業態語を追加、(C)保留、を提示。ユーザーから「Aの場合、検索時間はどれくらい増えるか」と質問があったため、実際にプロトタイプ実装して実測(下記)した上で、ユーザー了承を得てAを採用した。

### 修正
`lib/googleMaps.js`の`searchNearbyCafes()`を、keyword検索とtype=restaurant検索(keyword無し)を`Promise.all`で並列実行し、結果を`place_id`でマージ(重複除去)する形に変更。半径分割(tiering)は両方の検索に適用する(type=restaurant側も、広い半径では知名度で取りこぼす可能性があるため)。営業時間による絞り込みは既存通り後段(shopSearch.js)で行う。

### 検索時間への影響(実測)
| シナリオ | 変更前 | 変更後 | 差分 |
|---|---|---|---|
| 久留米市江戸屋敷・20km(一部キャッシュ有) | 6.6秒 | 7.0秒 | +0.4秒 |
| 長崎市・20km(未検索地点) | 4.7秒(コールド) | 5.9秒(一部キャッシュ有) | 目安+1.2秒 |
| 北九州市・20km(未検索地点、変更後のみ) | ― | 7.4秒(完全コールド) | ― |

候補プールはこのケースで51件→109件と倍近くに増えたが、Nearby Search自体はkeyword枝・type枝が並列実行されるため検索段階の時間はほぼ増えず、下流の候補処理(Place Details・ルート算出)も既に全店舗並列化済みのため、件数の増加ほど時間は伸びなかった(+0.4〜1.5秒程度)。候補プール倍増に伴うGoogle API同時リクエスト数の増加(レート制限リスク)は現状の利用規模では未検証・未顕在化。

### 検証
- 単体テスト追加(`test/googleMaps.typeMerge.test.js`): keywordに一致しなくてもtype=restaurantで見つかる店がマージされることを検証。既存の`test/googleMaps.pagination.test.js`・`test/googleMaps.radiusTiers.test.js`はkeyword枝・type枝の並列実行を踏まえて更新(type枝は1ページで完結させ、pagetoken付きリクエストをkeyword枝の継続と識別できるようにした)。全52件パス。
- 実データで「三福ラーメン」がkeyword検索には出ずtype=restaurant検索には出ることを確認。ただし往復ルート距離が24.0kmと算出され、希望距離20kmの距離フィルタで別途除外されることも判明(Action Items「waypoint迂回による距離超過」参照、今回は対応しない)。

## 2026-08-19(続き4): 検索半径が広い場合にNearby Searchの60件上限で候補が漏れる不具合を修正
ユーザー報告: 久留米市江戸屋敷起点・土曜6:30・希望距離50kmで「うどん·そば処 あずみうどん 大刀洗本店」が候補に入らない。

### 調査の経緯
最初は希望距離40kmでの検証しか行っておらず、「帰り道のoffset waypoint迂回で往復距離が40kmをわずかに超えて距離フィルタに引っかかる」という別原因(下記参照)を見つけて報告したが、ユーザーから「希望距離は50kmにしている」と訂正を受け、50kmで再調査した。

50kmで検索すると、この店舗はそもそも`searchNearbyCafes()`のNearby Search結果(60件)に含まれていないことが判明した。

### 原因
Google Places Nearby Search APIは1クエリにつき最大60件(20件×3ページ)までしか返さず、その60件はGoogle側の「知名度」順に選ばれる。希望距離50km(検索半径25km)では、久留米市中心部・鳥栖・佐賀方面の知名度の高いチェーン店(ジョイフル・すき家等)が競合として大量に加わり、60件の枠から「あずみうどん」のような近場(直線14.5km)だが知名度で劣る独立系の店舗が押し出されてしまっていた。実際に、同じ地点・同じkeywordで半径を15km・17.5km・20kmに絞って検索すると、この店舗は60件の中に含まれることを確認した(半径25kmの単一クエリでのみ漏れる)。

### 修正
ユーザー提案(「半径20km以内の候補の中で半径10kmの候補を除外する、等の方法で候補を絞れないか」)を採用し、`lib/googleMaps.js`の`searchNearbyCafes()`を半径分割(tiering)方式に変更した。
- 検索半径が`NEARBY_SEARCH_RADIUS_TIER_THRESHOLD_METERS`(15000m)以下の場合は従来通り単一半径のみで検索(挙動・コスト変更なし)。
- 半径がしきい値を超える場合は、フル半径に加えて`NEARBY_SEARCH_RADIUS_TIER_RATIOS`(`[0.4, 0.7]`)倍の半径でも追加でNearby Searchを実行し(3半径・各最大3ページ、`Promise.all`で並列)、結果を`place_id`でマージ(重複除去)する。小さい半径のクエリは競合する店舗数が少ないため、近場の店舗が60件の枠に残りやすくなることを利用している。

### 検証
- 単体テスト追加(`test/googleMaps.radiusTiers.test.js`): 半径がしきい値以下なら単一半径のみで検索すること、しきい値超過時は複数半径で検索し重複を除いてマージすることを検証。既存のページネーション系テスト(`test/googleMaps.pagination.test.js`)はしきい値をInfinityにモックして従来通りの単一半径での挙動を維持。全51件パス。
- 実データ(久留米市江戸屋敷起点・土曜6:30・希望距離50km)で「あずみうどん」(往復41.2km)が候補に含まれることをローカル・本番の両方で確認。検索時間は約7.7秒で、並列化前後で大きな悪化は無い。
- 本番デプロイ後もcycling-tracking-appへの影響が無いことを確認。

### 副次的に判明した別の要因(希望距離40kmの場合)
希望距離を40kmにした場合は、この店舗はNearby Search自体には含まれるが、別の理由で除外されることも分かった。直線距離14.5kmの往復ルートは、waypoint無しの直接ルートなら37.7kmで収まるが、復路の「同じ道を往復しない」ためのoffset waypoint(2026-08-19続き3で追加)を適用すると41.2kmまで伸び、希望距離40kmの距離フィルタに引っかかって除外される。倍率は1.18倍で遠回り判定(1.5倍超)には該当しないため、waypoint版がそのまま採用されてしまう。これは今回のNearby Search上限問題とは独立した別の境界事例であり、現時点では対応保留(Action Items参照)。

## 2026-08-19(続き3): 検索速度改善・復路の往復重複ルート修正・ルート表示ボタンの明示化
ユーザーからの改善提案3件に対応。

### 1. 候補店舗検索の高速化
`services/shopSearch.js`の`searchCandidateShops()`が、Nearby Search結果(最大60件)を`for...of`ループで1件ずつ`await`していた(Place Details取得→DB反映→営業時間判定→未キャッシュならルート算出、を全て逐次実行)。キーワード拡充で候補プールが増えたこともあり、地点によっては数十秒かかっていた。

各店舗の処理は互いに独立しているため、`Promise.all(preCandidates.map(async (place) => {...}))`で全店舗まとめて並列実行するように変更(`null`を返した店舗は`.filter()`で除外)。同様に`services/routeBuilder.js`の`buildRoundTripRoute()`でも、行き・帰りのルート取得(`Promise.all`)、標高プロファイルのサンプリング(`Promise.all`)を並列化。

実データ(未キャッシュの新規地点、希望距離20km)で検証したところ、候補20件の検索が約7秒で完了(体感でも大幅に改善)。既にルート情報がキャッシュ済みの地点ではさらに速い。

### 2. 復路が同じ道を往復してしまう問題の修正
ユーザー報告: 復路だけ同じ道を行って戻るような不自然なルートになることがあった。`routeBuilder.js`は帰り道にoffset waypointを与えて行きと別ルートにする仕組みだが、waypoint周りの道路事情によっては「waypointへ寄り道して同じ道を戻ってくる」経路が生成されることがある。

`hasSignificantBacktrack(path)`という判定関数を追加し、既存の「遠回り倍率(1.5倍超)」チェックに加えて、帰りルートの経路点列に自己重複(往復)が無いかもチェックするようにした。検知した場合は行きと同様、waypoint無しの直接ルートにフォールバックする。

**判定ロジックの調整に注意**: 当初は「インデックスが一定以上離れた2点の直線距離が40m未満」で判定していたが、実データで検証したところ交差点・カーブ付近で点が密集する箇所(実際には往復ではない)を誤検知することが判明(空港アクセス道路のループ等、無害だが誤検知しやすい)。「経路に沿って実際に進んだ累積距離が300m以上進んでいるにも関わらず、直線距離では40m未満まで戻ってきている」場合のみ折り返しとみなすよう変更し、模擬的な「1kmの行き止まり往復」では検知しつつ、実ルートでの誤検知が消えることを確認した。

### 3. 候補店舗カードに「推奨ルート表示」ボタンを追加
`src/mobile/src/screens/ShopCandidatesScreen.js`の`ShopCard`が、カード全体をタップするとルート表示に遷移する仕様だったが、初見のユーザーには分かりにくいとの指摘。カード全体のタップ動作を廃止し、カード下部に緑色の「推奨ルート表示」ボタンを明示的に配置した。

### 検証
- バックエンド: `hasSignificantBacktrack`の単体テストを追加(往復1kmのシミュレーション経路で検知、一方通行の経路・近接点の密集では非検知)。並列化に伴い`test/routeBuilder.integration.test.js`の1件(呼び出し順序に依存していたテスト)を、呼び出し順ではなく「行き(origin=start)向けの呼び出しだけ」を絞り込む形に修正。全49件パス。
- ローカルサーバーで実データ検証(未キャッシュ地点での検索・ルート生成)。
- バックエンドを本番(`https://breakfast.217-142-249-10.nip.io`)にデプロイ済み。デプロイ中もcycling-tracking-appへの影響が無いことを確認。
- Androidアプリを再ビルド(`breakfast-ride-planner-preview-route-btn-20260819.apk`、本番URL向けpreviewプロファイル)。まだ実機インストール・実機での動作確認はしていない(次回、実機にインストールして確認予定)。

### WSL2ローカルビルドで新たにハマった点(教訓)
今回、しばらく間隔を空けてから同じWSLビルド環境(`~/build/breakfast-ride-planner-mobile`)で`eas build --local`を実行したところ、以前は問題なかった箇所で2つ新たにつまずいた。次回の参考のため記録する。
- **「git repositoryが無い」エラー**: `eas build --local`はプロジェクトディレクトリがgitリポジトリであることを要求する。このWSLビルド用ディレクトリはWindows側からrsyncでソースだけ同期しており`.git`を持っていなかった(以前のセッションでは存在していたはずだが、失われていた)。`cd ~/build/breakfast-ride-planner-mobile && git init && git add -A && git commit -m ...`で解決(このgitリポジトリはビルド専用のローカルなものでよく、Windows側の本体リポジトリとは無関係)。
- **「SDK location not found」エラー**: `wsl -d Ubuntu -- bash -lc "..."`のように非対話シェルでコマンドを実行すると、Ubuntu標準の`.bashrc`冒頭にある`[ -z "$PS1" ] && return`によって`ANDROID_HOME`等の環境変数を設定している行(`.bashrc`後半)が読み込まれない。ビルドコマンドの直前で`export ANDROID_HOME=$HOME/Android/Sdk`等を明示的に設定することで解決。

## 2026-08-19(続き2): 検索キーワードを拡充
ユーザー指示で、Nearby Searchのkeywordに「朝食」「朝ごはん」「朝御飯」「朝ご飯」を追加(既存の「モーニング」「breakfast」と合わせて計6語)。`lib/googleMaps.js`の`searchNearbyCafes()`のkeywordを`'モーニング breakfast 朝食 朝ごはん 朝御飯 朝ご飯'`に変更。

実データ(久留米市江戸屋敷起点・土曜6:30・40km)で確認したところ、「朝ごはん屋 まるこ」に加えて松屋・すき家・モスバーガー・食堂系などモーニング/朝食メニューがある店舗も候補に含まれるようになった(想定通り。キーワード拡充によりカフェ的な店だけでなく朝食を提供するチェーン店・食堂も拾うようになったため、候補の毛色が少し広がる点は把握しておくこと)。テスト46件パス、本番デプロイ済み、cycling-tracking-appへの影響なしを確認。

## 2026-08-19(続き): `type=cafe`指定により「レストラン」分類のモーニング店が候補から漏れる不具合を修正
ユーザー報告: 久留米市江戸屋敷起点・土曜6:30スタートで「朝ごはん屋 まるこ」が候補に入らない。

### 原因
`lib/googleMaps.js`の`searchNearbyCafes()`がNearby Searchに`type=cafe`を指定していたが、「朝ごはん屋 まるこ」はGoogle Places上で`types: ["restaurant","food","point_of_interest","establishment"]`と分類されており`cafe`を含まない。`type`はGoogle側の構造化フィルタで、keywordのテキスト関連度マッチとは別にハードに絞り込まれるため、`type=cafe`を指定している限りこの店舗は(ページネーションを増やしても)絶対に候補プールに入らない。実際に、`type=cafe`を外して同じkeyword("モーニング breakfast")・同じ地点・同じ半径で検索したところ即座に見つかることを確認した。

### 修正
`searchNearbyCafes()`から`type: 'cafe'`パラメータを削除し、keywordによるテキスト関連度マッチのみで検索するように変更。仕様書も「カフェ・モーニング提供店」と両方を対象にしており、type縛りは元々の意図より狭すぎた。

### 検証
- 実データ(久留米市江戸屋敷起点・土曜6:30・希望距離40km)で「朝ごはん屋 まるこ」(往復37.4km、営業時間07:00〜11:00)が候補に含まれることをローカル・本番の両方で確認
- 本番デプロイ中もcycling-tracking-appへの影響が無いことを確認
- 既存テスト46件は`type`パラメータの有無を検証していなかったため、変更後も全件パス(パラメータ自体の単体テストは追加していない。将来またパラメータ構成を変える際は、`lib/googleMaps.js`のNearby Search呼び出しパラメータを直接アサートするテストを足すことを検討)
...
- **復路のoffset waypoint迂回が原因で、希望距離ギリギリの店舗が距離フィルタに引っかかって除外されることがある**(2026-08-19時点、保留)。「同じ道を往復しない」ための帰り道のoffset waypoint(`RETURN_ROUTE_WAYPOINT_OFFSET_RATIO`)は往復距離を1〜2割程度押し上げるため、直接ルートなら希望距離以内でも、waypoint適用後に希望距離を超えて除外されるケースがある(実例: あずみうどん大刀洗本店、直接37.7km→waypoint適用後41.2km、希望距離40kmだと除外)。倍率が遠回り判定(1.5倍超)の閾値内に収まる限りこの問題は起こりうる。対応案: `buildRoundTripRoute()`にユーザーの希望距離を渡し、waypoint適用後に希望距離を超える場合は直接ルートにフォールバックする、等。現状は少数の境界事例として保留。
