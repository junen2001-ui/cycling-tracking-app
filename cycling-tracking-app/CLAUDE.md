...
## 次に取り組むこと
- **最優先(2026-09-02時点)**: コース制対応の残作業 — (1)管理画面のPlaywright検証(コース別URL絞り込み・拡張Excelインポートダイアログ・Excelエクスポート・ゴール済み折りたたみの見た目/操作確認)、(2)運営マニュアル(Artifact)の作成(3コース分の画面URL・出走時刻設定・**受付での電話番号手動紐付け手順**・ゴール済み一覧/コース逸脱アラートの見方・Excelエクスポート手順を含む)。いずれもユーザーの明示的な指示があれば着手する。**(3)モバイルの実機ビルドは2026-09-02に完了(`build/mobile-preview-course-20260902.apk`)。残るは実機での動作確認のみ**(コース確認バナー・コース逸脱アラート〈バイブ+警告音〉・新アイコン〈VC福岡ロゴ〉。手順は`progress/progress.md`の2026-09-02セクション参照)。詳細は`progress/progress.md`の2026-08-28〜09-02セクションと計画ファイル`C:\Users\jun_e\.claude\plans\idempotent-riding-cascade.md`を参照。
...
- 実機確認済みの最新ビルド: `mobile-preview-autostop-20260828.apk`(`killServiceOnDestroy: false`化・`AUTO_STOP_HOUR_JST`自動停止・ウォッチドッグのハートビート修正を反映)。**その後のコース制対応一式(コース確認バナー・コース逸脱アラート〈`Vibration`+`expo-audio`で`assets/deviation-alert.wav`を再生〉・`GET /api/participants/me`)とアイコン変更(VC福岡ロゴ)を反映した `mobile-preview-course-20260902.apk` を2026-09-02にWSL2ローカルビルドで作成済み**(APKに埋め込まれたAPI URL・Hermesバンドル内の`course-deviation`/`participants/me`ハンドラ・警告音アセット`res/CR.wav`〈84716バイト、元ファイルと一致〉を確認済み。**実機での動作確認は未実施**)。詳細は `progress/progress.md` の2026-08-25〜2026-09-02の各セクション参照。
